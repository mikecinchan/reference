...
export default function EditEntry({ entry, onClose }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      try {
        const imageRef = ref(storage, entry.imageUrl);
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-100">
        <div className="p-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">Edit Entry</h2>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleUpdate} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Image {image && <span className="text-blue-600">(New image selected)</span>}
              </label>
              <input
                type="file"
                accept="image/*"
            ...
            <div className="flex gap-3 justify-between pt-2">
              <button
                ...
                disabled={loading}
                className="px-6 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-all duration-200 font-semibold shadow-lg hover:shadow-xl flex items-center gap-2"
              >
